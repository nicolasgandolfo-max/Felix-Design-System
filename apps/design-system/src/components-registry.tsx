...
export const SHOWCASES: Record<string, Showcase> = {
  ...
  bubble: {
    ...
    usage: {
      es: "El globo que envuelve el texto de un mensaje en una conversación. Importalo desde @felix/ui y elegí el tono con variant: default (turquesa, tus mensajes), secondary, muted (la otra persona), tinted, outline, ghost (sin marco, ideal para respuestas de IA o markdown) o destructive (error). Se ajusta al contenido: limitá el ancho desde el contenedor o dejá que Message lo haga.",
  ...
  marker: {
    ...
    usage: {
      es: "Una línea de baja jerarquía dentro de una conversación que no es un mensaje: un evento del sistema o un separador de fecha. Importalo desde @felix/ui y elegí variant: default (ícono + texto), separator (texto centrado entre dos líneas, ideal para 'Hoy') o border (ícono + texto con una línea inferior). El ícono por defecto es GitBranch de Phosphor; reemplazalo con icon o pasá null para omitirlo.",
  ...
  attachment: {
    ...
    usage: {
      es: "El adjunto que acompaña un mensaje o el compositor del chat: un comprobante en PDF, una foto del ticket. Importalo desde @felix/ui y pasale name y meta (formato · peso). Elegí size (sm, md o lg), state (default, error o loading, que muestra spinner y barra de progreso con progress) y type (file o image; con image y lg se vuelve una tarjeta vertical con miniatura). El botón de cerrar dispara onRemove; etiquetalo con removeLabel.",
  ...
  message: {
    ...
    usage: {
      es: "Una entrada de la conversación: un avatar opcional más una columna con encabezado (quién escribe), un Bubble y un pie (hora o estado de lectura). Importá Message, MessageAvatar, MessageContent, MessageHeader y MessageFooter desde @felix/ui. Con align='start' el avatar va a la izquierda y el contenido se alinea al inicio (la otra persona); con align='end' se invierte (tus mensajes). Escribí siempre el avatar primero en el JSX: align se encarga del orden visual.",
        ...
        <Demo label="Estado vacío" col>
          <MessageScroller style={{ width: 380, height: 600 }}>
            ...
            <MessageScrollerEmpty title="Listo para empezar">
              Escribí tu consulta y<br />te respondemos enseguida.
