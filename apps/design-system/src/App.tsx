...
function NotFound() {
  ...
  return (
    <section className="sec flush">
      ...
      <h2 className="h2">
        {tr("Página no encontrada", "Page not found", "Página não encontrada")}
      </h2>
      <p className="lead">
        {tr(
          "Esa ruta no existe en el sistema.",
          "That route doesn't exist in the system.",
          "Essa rota não existe no sistema."
        )}
      </p>
      <Link className="btn-link" to="/">
        {tr("← Volver al inicio", "← Back to overview", "← Voltar ao início")}
      </Link>
    </section>
  );
}
    ...
    <>
      ...
      <Routes>
        ...
        <Route element={<VoiceToneLayout />}>
          ...
          <Route
            path="voz-y-tono/version-anterior"
            element={<PlazaEditorial />}
          />
        ...
        <Route
          path="editorial"
          element={<Navigate to="/voz-y-tono/version-anterior" replace />}
        />
        ...
        <Route element={<Layout />}>
          ...
          <Route
            path="iconografia"
            element={<Navigate to="/ilustraciones" replace />}
          />
          <Route path="componentes" element={<ComponentsIndex />} />
          <Route
            path="componentes/:groupSlug"
            element={<ComponentGroupPage />}
          />
          ...
          <Route
            path="design-md"
            element={<Navigate to="/markdown" replace />}
          />
